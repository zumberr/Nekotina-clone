...
module.exports = {
    name: "resetlevel",
    usage: "resetlevel | resetlvl <@member | memberID>",
    aliases: ["resetlvl"],
    cooldown: 2,
    example: "resetlevel | resetlvl <@Discord#0000 | 1234567890>",
    ownerOnly: false,
    UserPerms: ["SEND_MESSAGES"],
    ClientPerms: ["SEND_MESSAGES", "EMBED_LINKS"],
    description: "Resetear el nivel de usuario mencionado.",
   ...
   try {
    ...
    if(xpranker === true) {
        ...
        if(!member) {
            ...
            client.database.delete(`level_${message.guild.id}_${member.id}`)
    
            return message.reply({
                embeds: [new MessageEmbed()
                .setColor("#fbd9ff")
                .setAuthor({ name: `El perfil de ${member.user.tag} ha recibido cambios` })
                .setDescription(`${message.member} le ha restablecido el nivel al usuario ${member}`)
                ]
            })
        } catch(error) {
            return message.reply({
                ...
                .setDescription("Ha ocurrido un error al intentar restablecer el nivel del usuario mencionado.")
                ]
            })
        }
    } else {
