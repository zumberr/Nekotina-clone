...
module.exports = {
    name: "setconfessions",
    usage: "setconfessions | setconfs <#channel | channelID>",
    aliases: ["setconfs"],
    cooldown: 2,
    example: "setconfessions | setconfs #suggestions",
    ownerOnly: false,
    UserPerms: ["MANAGE_CHANNELS"],
    ClientPerms: ["SEND_MESSAGES", "EMBED_LINKS", "MANAGE_CHANNELS"],
    description: "Establecer el canal de confesiones del servidor.",
   ...
   try {
    ...
    if(!channel) {
        ...
        await database.set(`confessions_${message.guild.id}`, channel.id)

        return message.reply({
            embeds: [new MessageEmbed()
            .setColor("#fbd9ff")
            .setAuthor({ name: "Se ha modificado la configuración del servidor" })
            .setDescription(`Se ha modificado el canal de confesiones del servidor.\nEste el nuevo canal de confesiones del servidor: ${channel}`)
            ]
        })
    } catch(error) {
        return message.reply({
