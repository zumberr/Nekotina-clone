...
module.exports = {
    name: "setsuggestions",
    usage: "setsuggestions | setsuggests <#channel | channelID>",
    aliases: ["setsuggests"],
    cooldown: 2,
    example: "setsuggestions | setsuggests #suggestions",
    ownerOnly: false,
    UserPerms: ["MANAGE_CHANNELS"],
    ClientPerms: ["SEND_MESSAGES", "EMBED_LINKS", "MANAGE_CHANNELS"],
    description: "Establecer el canal de sugerencias del servidor.",
   ...
   try {
    ...
    if(!channel) {
        return message.reply({
            ...
            .setColor("#fbd9ff")
            .setAuthor({ name: "Error Code: 4718" })
            .setDescription(`No se ha proporcionado la mención/ID del canal.\n\nUso corecto del comando:\n\` ${this.usage} \``)
            ]
        })
    } else try {
        await database.set(`suggestion_${message.guild.id}`, channel.id)

        return message.reply({
            embeds: [new MessageEmbed()
            .setColor("#fbd9ff")
            .setAuthor({ name: "Se ha modificado la configuración del servidor" })
            .setDescription(`Se ha modificado el canal de sugerencias del servidor.\nEste el nuevo canal de sugerencias del servidor: ${channel}`)
            ]
        })
    } catch(error) {
        return message.reply({
